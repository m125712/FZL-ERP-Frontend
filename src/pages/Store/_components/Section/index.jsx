...
export default function Index({ type, accessor }) {
	...
	const { data, isLoading, deleteData } = useMaterialSection(type);
	const info = new PageInfo(
		`Store (${type.toUpperCase()}) / Section`,
		`/store-${type}/section`,
		accessor
	);

	const haveAccessAccessor = useAccess(accessor);
				...
				cell: (info) => {
					return (
						<EditDelete
							...
							showUpdate={haveAccessAccessor?.includes('update')}
							showDelete={haveAccessAccessor?.includes('delete')}
						/>
					);
				},
			},
	...
	return (
		<>
			<ReactTable
				title={info.getTitle()}
				handelAdd={handelAdd}
				accessor={haveAccessAccessor.includes('create')}
				data={data}
				columns={columns}
			/>
