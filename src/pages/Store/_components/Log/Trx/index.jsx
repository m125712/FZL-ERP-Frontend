...
export default function Index({ type, accessor }) {
	...
	const { data, isLoading, deleteData } = useMaterialTrx(
		type,
	...
	const info = new PageInfo(
		`Store (${type.toUpperCase()}) / Transfer`,
		`/store-${type}/log`
	);
	...
	const columns = useMemo(
		() => [
			...
			},
			{
				...
				enableSorting: false,
				hidden: !(
					haveAccess?.includes('update_log') ||
					haveAccess?.includes('delete_log')
				),
				width: 'w-24',
				cell: (info) => {
					return (
						<EditDelete
							...
							showUpdate={haveAccess?.includes('update_log')}
