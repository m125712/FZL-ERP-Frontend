	...
	const columns = useMemo(
		() => [
				...
				cell: (info) => {
					...
					return (
						<div className='flex space-x-1'>
							...
							<StatusButton size='btn-xs' value={is_cash === 1 ? 0 : 1} />
						</div>
					);
				},
			},
			...
			},
			{
				...
				enableColumnFilter: false,
				cell: (info) => {
					const { marketing_priority, factory_priority } = info.row.original;
			...
			},
			{
				...
				header: 'Status',
				enableColumnFilter: false,
				cell: (info) => <StatusButton size='btn-sm' value={info.getValue()} />,
			},
			{
