...
export default function Index({ type, accessor }) {
	...
	const info = new PageInfo(
		type === 'maintenance'
			? `Store (${type.toUpperCase()}) / Transfer Against Issue`
			: `Store (${type.toUpperCase()}) / Transfer Against Order`,
		`/store-${type}/log`
	);
	...
	const columns = useMemo(
		() => [
...

			{
				...
				enableSorting: false,
				hidden: !(
					haveAccess?.includes('update_log_against_order') ||
					haveAccess?.includes('delete_log_against_order')
				),
				width: 'w-24',
				cell: (info) => {
					return (
