...
export default function Header({
	...
	const { order_info_id, order_description_uuid } = useParams();

	const { value: item } = useFetch(`/order-properties/by/item`);
	const { value: zipper_number } = useFetch(
		`/order-properties/by/zipper_number`
	);
	const { value: end_type } = useFetch(`/order-properties/by/end_type`);
	const { value: lock_type } = useFetch(`/order-properties/by/lock_type`);
	const { value: puller_type } = useFetch(`/order-properties/by/puller_type`);
	const { value: color } = useFetch(`/order-properties/by/color`);
	const { value: hand } = useFetch(`/order-properties/by/hand`);
	const { value: special_requirement } = useFetch(
		`/order-properties/by/special_requirement`
	);
	const { value: stopper_type } = useFetch(
		`/order-properties/by/stopper_type`
	);
	const { value: coloring_type } = useFetch(
		`/order-properties/by/coloring_type`
	);
	const { value: slider } = useFetch(`/order-properties/by/slider`);
	const { value: top_stopper } = useFetch(`/order-properties/by/top_stopper`);
	const { value: bottom_stopper } = useFetch(
		`/order-properties/by/bottom_stopper`
	);
	const { value: logo_type } = useFetch(`/order-properties/by/logo_type`);
