...
export default function Index({
	...
	const { user } = useAuth();
	const MAX_QUANTITY =
		updateTeethMoldingLog?.m_teeth_molding +
		updateTeethMoldingLog?.used_quantity;
	const schema = {
		...RM_MATERIAL_USED_EDIT_SCHEMA,
		used_quantity:
	...
	useFetchForRhfReset(
		`material/used/${updateTeethMoldingLog?.id}`,
	...
	const onSubmit = async (data) => {
		...
		if (updateTeethMoldingLog?.id !== null) {
			const updatedData = {
				...data,
				material_name: updateTeethMoldingLog?.material_name,
			...
			await useUpdateFunc({
				uri: `/material/used/${
					updateTeethMoldingLog?.id
				}/${updateTeethMoldingLog?.material_name.replace(/[#&/]/g, "")}`,
	...
	return (
		<AddModal
			id={modalId}
			title={`Teeth Molding Log of ${updateTeethMoldingLog?.material_name}`}
